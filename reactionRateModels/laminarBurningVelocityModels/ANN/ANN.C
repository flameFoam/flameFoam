...
()
{
    ...
    {	
    	...
    	Y0out_.set(i, new volScalarField(Y0out_names_[i], tanh(L0_[i] + B0_[i])));
	}
	...
	{   
		...
		Y1out_.set(i, new volScalarField(Y1out_names_[i], L1_[i] + B1_[i]));
		Y1out_[i].max(0);
	}
	...
	{   
		...
		Y2out_.set(i, new volScalarField(Y2out_names_[i], L2_[i] + B2_[i]));
		Y2out_[i].max(0);
	}
	...
	{	
		...
		Y3out_.set(i, new volScalarField(Y3out_names_[i], L3_[i] + B3_[i]));
		Y3out_[i].max(0);
	}
